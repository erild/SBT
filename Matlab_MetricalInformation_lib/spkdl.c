/*
function d=spkdl(stime,sstart,send,cost)
...
%
%  STIME	      list of spike times (row vector) (in ms)
%  SSTART      list of trial start indices (row vector, length: # stimulus XMPsentations) - position indexes in matlab STIME vector (eg 1:n)
%  SEND        list of trial end indices (row vector, length: same as SSTART); if a trial has no
%               spikes in it, the SEND value should be one less than the corresponding SSTART value - position indexes in matlab STIME vector (eg 1:n)
